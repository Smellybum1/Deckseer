...
class AccuracyScenario:
    ...
    @classmethod
    def from_dict(cls, raw: Any, label: str) -> "AccuracyScenario":
...
def load_accuracy_scenarios(path: Path) -> tuple[AccuracyScenario, ...]:
    try:
        with path.open("r", encoding="utf-8") as handle:
            raw = json.load(handle)
    except FileNotFoundError as exc:
        raise DataError(f"Accuracy scenario manifest not found: {path}") from exc
    except json.JSONDecodeError as exc:
        raise ValidationError(f"{path} is not valid JSON: {exc}") from exc

    data = _require_mapping(raw, "accuracy manifest")
    if data.get("manifest_type") != "accuracy_scenarios":
        raise ValidationError(f"{path}.manifest_type must be accuracy_scenarios")
    scenarios = _require_list(data.get("scenarios"), f"{path}.scenarios")
    parsed = tuple(AccuracyScenario.from_dict(item, f"{path}.scenarios[{index}]") for index, item in enumerate(scenarios))
...
def build_accuracy_report(data: DeckseerData, *, manifest_path: Path) -> dict[str, Any]:
    scenarios = load_accuracy_scenarios(manifest_path)
...
def _score_scenario(scenario: AccuracyScenario, data: DeckseerData) -> dict[str, Any]:
    try:
        run = RunState.from_json_file(scenario.path)
