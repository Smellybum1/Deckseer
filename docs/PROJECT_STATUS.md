...
- Empirical intake queue for pending source review notes that are not active audit evidence.
...
1. Deckseer Exporter Mod implementation readiness: Deckseer can now inspect static exporter status files and recommend from confirmed card reward exporter files, but the in-game exporter mod and watch mode have not been implemented.
