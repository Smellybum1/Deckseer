...
- Private Relic Choice V1 scorer and fixtures for manual `screen_type: "relic_reward"` JSON, with no public relic advice CLI yet.
...
5. Broader advice modules: relic choice is the approved first surface. `docs/RELIC_CHOICE_DESIGN.md` defines the design boundary, `docs/RELIC_METADATA_SEED_PLAN.md` documents the seed metadata, and the private scorer is covered by `tests/test_relic_choice.py`. The next packet should add a public `recommend-relic` CLI around that scorer.
