...
- Relic choice design document for the first approved broader advice surface, still with no runtime behavior changes.
- Relic metadata seed plan and reviewed V1 metadata on the existing three seed relics, with no relic advice runtime behavior yet.
...
5. Broader advice modules: relic choice is the approved first surface. `docs/RELIC_CHOICE_DESIGN.md` defines the design boundary, and `docs/RELIC_METADATA_SEED_PLAN.md` documents the seed metadata. The next packet should be a private relic choice scorer before public CLI behavior is added.
