...
- Read-only import from proposed Deckseer Exporter JSON files, with exporter metadata and caveats kept outside scoring. `inspect-export` accepts static exporter status files and card reward files; `recommend-export` remains card-reward only. The proposed future relic reward exporter contract is documented but not yet adapter-enabled.
