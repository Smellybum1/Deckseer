...
3. Deckseer Exporter Mod next stage: the static in-game spike now works and is documented in `docs/EXPORTER_STATIC_MOD_SPIKE.md`. Deckseer can inspect the real mod-written `exporter_status` file, recommend from confirmed card/relic exporter JSON fixtures, and run `deckseer exporter-toolchain-preflight --format text` to repeat local readiness checks. `docs/EXPORTER_CARD_REWARD_COMPILE_PROBE.md` confirms a narrower public API set; the next exporter packet should either install-check the `v0.2.0` diagnostic status build with explicit approval, or design how to prove `RewardsSet.Rewards` and `CardReward.Cards` correspond to the visible card reward screen. Watch mode has not been implemented.
