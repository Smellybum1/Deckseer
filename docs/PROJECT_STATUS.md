...
3. Deckseer Exporter Mod next stage: the static in-game spike now works and is documented in `docs/EXPORTER_STATIC_MOD_SPIKE.md`. Deckseer can inspect the real mod-written `exporter_status` file, recommend from confirmed card/relic exporter JSON fixtures, and run `deckseer exporter-toolchain-preflight --format text` to repeat local readiness checks. The next exporter packet should identify safe normal mod-accessible APIs for visible card reward export before writing any live run state. Watch mode has not been implemented.
